...
class ILRLMTSAC(OffPolicyAlgorithm[ILRLMTSACConfig]):
    ...
    def train(
        ...
        for global_step in range(start_step, config.total_steps // envs.num_envs):
            ...
            if global_step > config.warmstart_steps:
                ...
                self, logs, td_err = self.update(batch.rl)
    ...
    @jax.jit
    def _update_inner(self, data: ReplayBufferSamples) -> tuple[Self, LogDict, Float[Array, "batch"]]:
        ...
        data_sort_indices = None  # <- split critic일 때 td_err unsplit에 필요
        ...
        # td_err unsplit (split_critic_losses인 경우 td_err shape: (num_tasks, per_task_batch))
        if self.split_critic_losses:
        ...
        critic_optim_logs = (
            {f"metrics/critic_{key}": value for key, value in self.critic.opt_state[0]._asdict().items()}
            if isinstance(self.critic.opt_state[0], PCGradState)
            else {}
        )
        actor_optim_logs = (
            {f"metrics/actor_{key}": value for key, value in self.actor.opt_state[0]._asdict().items()}
            if isinstance(self.actor.opt_state[0], PCGradState)
            else {}
        )

        il_logs: LogDict = {}
        il_batch = getattr(data, "il_batch", None)
        ...
        if (il_batch is not None) and (self.il_coef > 0.0):
            ...
            self, il_log_stack = jax.lax.scan(
                ...
                self,
                xs=None,
                length=self.il_update_per_step,  # <- 원래 코드에 il_updates_per_step 혼재되어 있었는데 여기선 config field명 사용
            )
        ...
        logs = {
            ...
            **il_logs,
            # (선택) td_err 통계도 같이 로깅 가능
            "data/td_abs_mean": jnp.mean(jnp.abs(td_err)),
        }
        return self, logs, td_err
    # --- (3) update: td_err도 반환 ---
    @override
    def update(self, data: ReplayBufferSamples) -> tuple[Self, LogDict, Float[Array, "batch"]]:
